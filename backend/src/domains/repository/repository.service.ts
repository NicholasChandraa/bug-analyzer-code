...
import {
  repositoryRepo,
  type RepositoryRow,
  type CodebaseSyncRow,
} from "./repository.repo.js";

export class RepositoryNotFoundError extends Error {}
export class DuplicateSlugError extends Error {}

const toRepositoryResponseDTO = (
  repo: RepositoryRow,
): RepositoryResponseDTO => ({
  id: repo.id,
  name: repo.name,
...
const toCodebaseSyncResponseDTO = (
  log: CodebaseSyncRow,
): CodebaseSyncResponseDTO => ({
  id: log.id,
...
export const repositoryService = {
  registerRepository: async (
    ...
    if (existing)
      throw new DuplicateSlugError(`Slug "${data.slug}" is already in use`);

    // Validate local path existence
    const resolved = path.resolve(data.localPath);
    if (!fs.existsSync(resolved)) {
      throw new Error(
        `Local directory "${data.localPath}" does not exist on host PC`,
      );
    }
  ...
  syncRepository: async (
    ...
    for (const repo of targets) {
      ...
      // Local mode: verify folder exists (gak ada git clone/install lagi)
  ...
  resolveRepositoryBySlugInternal: async (
    slug: string,
  ): Promise<RepositoryResponseDTO> => {
    const repo = await repositoryRepo.getRepositoryBySlug(slug);
    if (!repo)
      throw new RepositoryNotFoundError(
        `Repository dengan slug "${slug}" belum terdaftar`,
      );
    return toRepositoryResponseDTO(repo);
  },

  listRepositoriesInternal: async (
    slugs?: string[],
  ): Promise<RepositoryResponseDTO[]> => {
    const repos =
      slugs && slugs.length > 0
        ? (
            await Promise.all(
              slugs.map((s) => repositoryRepo.getRepositoryBySlug(s)),
            )
          ).filter((r): r is RepositoryRow => r !== null)
        : await repositoryRepo.listRepositories();
    return repos.map(toRepositoryResponseDTO);
  },
  ...
  browseDirectories: async (
    ...
    try {
      entries = await fs.promises.readdir(resolvedPath, {
        withFileTypes: true,
      });
    } catch {
      throw new Error(
        `Directory "${resolvedPath}" does not exist or is not readable`,
      );
    }
    ...
    const normalizedParent =
      parent !== resolvedPath ? parent.replace(/\\/g, "/") : null;
